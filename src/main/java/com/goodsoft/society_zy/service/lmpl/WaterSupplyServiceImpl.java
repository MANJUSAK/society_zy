...
import com.goodsoft.society_zy.domain.dao.WaterSupplyDao;
...
import com.goodsoft.society_zy.service.supp.WaterSupplyServicelmplSupp;
...
public class WaterSupplyServiceImpl implements WaterSupplyService {
    @Resource
    private WaterSupplyDao waterSupplyDao;  //供电数据操作类
    @Resource
    ...
    @Resource
    private WaterSupplyServicelmplSupp waterSupplyServicelmplSupp;  //数据清洗辅助业务类
    //实例化excel读取工具类
     ...
     */
    @Override
    public <T> T addWaterSupplyAccountInfo(HttpServletRequest request, MultipartFile[] files) throws Exception {
        //设置文件编号
        ...
        sdData.addAll(set);
            /*获取上传excel文件数据 end*/
        //解析excel数据有效性，返回错误数据
        List<ExcelColumnInfo> errorList = this.waterSupplyServicelmplSupp.waterAccountInfo(sdData);
        int len = sdData.size();
        ...
        if (len > 0) {
            ...
            try {
                ...
                for (int i = 0; i < len; ++i) {
                    ...
                    String code="QAA"+bm+str+"01"+rq+al;
                    ...
                    this.waterSupplyDao.addWaterSupplyAccountInfo(sdData.get(i));
                }
                session.commit();
            } finally {
                session.close();
     ...
     */
    @Override
    public <T> T addWaterSupplyReadingInfo(HttpServletRequest request, MultipartFile[] files) throws Exception {
        //设置文件编号
        String uuid = this.uuid.getUUID().toString();
        //文件上传
        int arg = this.fileService.fileUploadService(files, "excel", uuid);
        ...
        sdData.addAll(set);
            /*获取上传excel文件数据 end*/
        //解析excel数据有效性，返回错误数据
        List<ExcelColumnInfo> errorList = this.waterSupplyServicelmplSupp.waterReadingInfo(sdData);
        int len = sdData.size();
        //判断解析数据是否满足正确格式数据，是存库，否删除原始文件
        if (len > 0) {
            //导出错误数据到excel
            ...
            try {
                ...
                for (int i = 0; i < len; ++i) {
                    ...
                    String rq =time.substring(4);
                    String bm="520402000000"; //西秀机构代码
                    String code="QAA"+bm+str+"02"+rq+al;
                    sdData.get(i).setColumn(code);
                    // System.out.println(sdData.get(i).getColumn()+">>"+sdData.get(i).getColumn1()+">>"+sdData.get(i).getColumn2()+">>"+sdData.get(i).getColumn3()+">>"+sdData.get(i).getColumn4()+">>"+sdData.get(i).getColumn5()+">>"+sdData.get(i).getColumn6()+">>"+sdData.get(i).getColumn7()+">>"+sdData.get(i).getColumn8()+">>"+sdData.get(i).getColumn9()+">>"+sdData.get(i).getColumn10()+">>"+sdData.get(i).getColumn11());
                    this.waterSupplyDao.addWaterSupplyReadingInfo(sdData.get(i));
                }
                session.commit();
            } finally {
                session.close();
